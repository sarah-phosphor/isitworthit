...
import { DayView, SearchView, GroupView, TeamView } from '../src/App'
...
check('TeamView', <TeamView teamId={tid} ctx={ctx} backLabel="Today" nav={nav} />, [tname, 'Current status', 'Past matches'])
check('SearchView', <SearchView ctx={ctx} query="" onQuery={() => {}} nav={nav} />, ['Search by team', 'pts'])
check('SearchView(filtered)', <SearchView ctx={ctx} query="united" onQuery={() => {}} nav={nav} />, ['pts'])
